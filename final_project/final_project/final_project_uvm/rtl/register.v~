module register(	
	...
	// TCR Register
	assign div_val_en = (wdata[11:8] < 4'h9);		// Condition
	always @(posedge clk or negedge rst_n) begin
		if (!rst_n) 
			tcr_reg 	<= TCR_DEFAULT;
		else begin
			if (sel_reg[0] && wr_en && div_val_en && !tcr_reg[0]) begin	// div_val changes when timer_en is LOW
				tcr_reg[11:8] 	<= wdata[11:8];
			end else begin
				tcr_reg[11:8] 	<= tcr_reg[11:8];
			end

			if (sel_reg[0] && wr_en && !tcr_reg[0]) begin			// div_en
				tcr_reg[1]	<= wdata[1];
			end else begin
				tcr_reg[1]	<= tcr_reg[1];
			end
	...
	// TISR Register
	assign compare_match = int_en && (count_val == {tcmp1_reg, tcmp0_reg});
	always @(posedge clk or negedge rst_n) begin
		if (!rst_n)
			tisr_reg                <= TISR_DEFAULT;
		else if (wr_en && sel_reg[6] && wdata[0])
			tisr_reg[0]	<= 1'b0;
		else if (compare_match) //&& timer_en)
			tisr_reg[0]	<= 1'b1;
		else
